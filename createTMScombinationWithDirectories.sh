				 ...
				 for (( r=0; r <= $(pow 2 $i); r++ )); do 
					...
					for (( c=0; c <=$(pow 2 $i); c++ )); do 
						...
						if [ -f "${bands[0]}/$i/$r/$c.png" ]; then 	# if file exists on destination merge and composite
							...
							if [ -f "$DestinationDirectory/$i/$r/$c.png" ]; then 
								# echo "file exists and it is combined *" 
								echo "zoomlevel $i processing" 
								# convert -channel RGB "${bands[0]}/$i/$r/$c.png" -set colorspace RGB -combine -set colorspace sRGB -transparent black "c.png"
								convert "${bands[0]}/$i/$r/$c.png" "$DestinationDirectory/$i/$r/$c.png" 
								# rm "c.png"

						 	else 	#else copy file to directory
								echo "zoomlevel $i does not exist and it is created"
						 	    cp "${bands[0]}/$i/$r/$c.png" "$DestinationDirectory/$i/$r/$c.png"
								# convert "${bands[0]}/$i/$r/$c.png" "$DestinationDirectory/$i/$r/$c.png" 
							    # convert  -channel RGB  "${bands[0]}/$i/$r/$c.png" -set colorspace RGB -combine -set colorspace sRGB  "$DestinationDirectory/$i/$r/$c.png"
							fi
						fi
					done 
					#*******
